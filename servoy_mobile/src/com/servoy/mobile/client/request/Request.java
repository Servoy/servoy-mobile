...
public class Request
{
	...
	/*
	 * Timer used to force HTTPRequest timeouts. If the user has not requested a timeout then this field is null.
	 */
	private final Timer timer;

	/*
	 * JavaScript XmlHttpRequest object that this Java class wraps. This field is not final because we transfer ownership of it to the HTTPResponse object and
	...
	{
		/*
		 * There is a strange race condition that occurs on Mozilla when you cancel a request while the response is coming in. It appears that in some cases the
		 * onreadystatechange handler is still called after the handler function has been deleted and during the call to XmlHttpRequest.abort(). So we null the
		 * xmlHttpRequest here and that will prevent the fireOnResponseReceived method from calling the callback function.
		 * 
		 * Setting the onreadystatechange handler to null gives us the correct behavior in Mozilla but crashes IE. That is why we have chosen to fixed this in
		 * Java by nulling out our reference to the XmlHttpRequest object.
		 */
		if (xmlHttpRequest != null)
		{
	...
	public boolean isPending()
	{
		...
		/*
		 * Because we are doing asynchronous requests it is possible that we can call XmlHttpRequest.send and still have the XmlHttpRequest.getReadyState method
		 * return the state as XmlHttpRequest.OPEN. That is why we include open although it is nottechnically true since open implies that the request has not
		 * been sent.
		 */
		switch (readyState)
		{
	...
	/*
	 * Method called when the JavaScript XmlHttpRequest object's readyState reaches 4 (LOADED).
	 */
	void fireOnResponseReceived(RequestCallback callback)
	{
		...
		/*
		 * We cannot use cancel here because it would clear the contents of the JavaScript XmlHttpRequest object so we manually null out our reference to the
		 * JavaScriptObject
		 */
